...
public class ResultBundleReader {
    ...
    public static TestResult[] readBundle(InputStream in) throws IOException {
        ...
        if (header != HEADER) throw new IOException("Wrong header: " + header);
        ...
        for(int i = 0; i < numTests; i++){
            ...
            String actualOutput = new String(dataIn.readNBytes(actualOutputLen));
            int expectedOutputLen = dataIn.readInt();
            String expectedOutput = new String(dataIn.readNBytes(expectedOutputLen));
            long runtimeMillis = dataIn.readLong();
            int errorLength = dataIn.readInt();
            String error = new String(dataIn.readNBytes(errorLength));
